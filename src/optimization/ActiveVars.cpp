...
/*
    ...
    false:not equal
*/
bool judge_set(std::set<Value*>set_a,std::set<Value*>set_b)
{
    //firstly we judge size
    if (set_a.size() != set_b.size())
        return false;
    //based on same size,we compare each element
    std::set<Value *>::iterator it_a, it_b;
    for (it_a = set_a.begin(), it_b = set_b.begin(); it_a != set_a.end(); it_a++,it_b++)
    {
        if (*it_a != *it_b)
        {
            return false;
...
void union_set_reserve(std::set<Value *> &set_a, std::set<Value *> set_b,std::set<Value*> set_c)
{
    for(auto item:set_c)
    {
...
void intersection_set(std::set<Value *> &set_a, std::set<Value *> set_b)
{
    for(auto item : set_b){
