...
%token <node> IDENTIFIER INTEGER FLOATPOINT LPARENTHESE RPARENTHESE LBRACE RBRACE LBRACKET RBRACKET ARRAY ADD SUB MUL DIV LT LTE GT GTE EQ NEQ ASSIN SEMICOLON COMMA ELSE IF INT RETURN VOID WHILE FLOAT MAIN
%type <node> type-specifier relop addop mulop declaration-list declaration var-declaration fun-declaration local-declarations compound-stmt statement-list statement expression-stmt iteration-stmt selection-stmt return-stmt expression simple-expression var additive-expression term factor integer float call params param-list param args arg-list program
...
| type-specifier MAIN LPARENTHESE params RPARENTHESE compound-stmt
{
    $$ = node("fun-declaration", 6, $1, $2, $3, $4, $5, $6);
}
